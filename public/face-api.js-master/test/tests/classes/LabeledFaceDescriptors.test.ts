...
describe('globalApi', () => {
  describe('LabeledFaceDescriptors', () => {
    ...
    it('JSON.stringify()', () => {
      expect(JSON.stringify(new LabeledFaceDescriptors(l1, [f1, f2]))).toBe(
        json
      );
      expect(
        JSON.stringify({ ld: new LabeledFaceDescriptors(l1, [f1, f2]) })
      ).toBe(`{"ld":${json}}`);
      expect(JSON.stringify([new LabeledFaceDescriptors(l1, [f1, f2])])).toBe(
        `[${json}]`
      );
    ...
    it('toJSON() => fromJSON()', () => {
      const ld = LabeledFaceDescriptors.fromJSON(
        new LabeledFaceDescriptors(l1, [f1, f2]).toJSON()
      );
